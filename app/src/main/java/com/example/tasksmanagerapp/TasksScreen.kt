...
fun TaskItem(task: Task, onToggleCompletion: () -> Unit, onDelete: () -> Unit) {
    val scale by animateFloatAsState(if (task.isCompleted) 1.05f else 1f, label = "")
    val backgroundColor = when (task.priority) {
        TaskPriority.BAIXA -> Color(0xFF34CE3A)
        TaskPriority.MEDIA -> Color(0xFFC2B22E)
        TaskPriority.ALTA -> Color(0xFFF32D40)
    }
    ...
    Column {
        ...
        Button(
            ...
            colors = ButtonDefaults.buttonColors(
                containerColor = Color(0xFF186E86),
